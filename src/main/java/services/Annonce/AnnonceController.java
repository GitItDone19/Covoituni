...
public class AnnonceController {
    ...
    private void modifierAnnonce(Annonce annonce) {
        try {
            ...
            // Récupérer le contrôleur et initialiser l'annonce
    ...
    private void handleReservation(Annonce annonce) {
        try {
            // 1. Récupérer le trajet pour vérifier les places disponibles
            Trajet trajet = trajetService.readById(annonce.getTrajetId());
            if (trajet == null) {
                showAlert("Erreur", "Trajet non trouvé", Alert.AlertType.ERROR);
                return;
            }

            // 2. Vérifier s'il y a des places disponibles
            if (trajet.getAvailableSeats() <= 0) {
                showAlert("Erreur", "Plus de places disponibles", Alert.AlertType.ERROR);
                return;
            }

            // 3. Afficher une boîte de dialogue avec les informations
            TextInputDialog dialog = new TextInputDialog();
            dialog.setTitle("Réservation");
            dialog.setHeaderText(String.format(
                "Places disponibles : %d\nAjouter un commentaire pour votre réservation",
                trajet.getAvailableSeats()
            ));
            dialog.setContentText("Commentaire:");

            // 4. Traiter la réservation
            Optional<String> result = dialog.showAndWait();
            result.ifPresent(comment -> {
                try {
                    Reservation reservation = reservationService.createReservation(
                        annonce.getId(),
                        Long.valueOf(1), // ID utilisateur fixe pour le moment
                        comment
                    );
                    
                    // 5. Afficher le message de succès avec les places restantes
                    showAlert(
                        "Succès",
                        String.format(
                            "Réservation créée avec succès\nPlaces restantes : %d",
                            trajet.getAvailableSeats() - 1
                        ),
                        Alert.AlertType.INFORMATION
                    );
                    
                    // 6. Recharger la liste des annonces
                    loadAnnonces();
            ...
            showAlert(
                "Erreur",
                "Erreur lors de la vérification des places disponibles: " + e.getMessage(),
                Alert.AlertType.ERROR
            );
        }
    }
    ...
    private int getCurrentUserId() {
        // Pour le moment, on retourne un ID fixe
        // À remplacer plus tard par l'ID de l'utilisateur connecté
        return 1;
    }
                ...
                if (empty || annonce == null) {
                    ...
                    try {
                        ...
                        if (trajet != null) {
                            ...
                            Label placesLabel = new Label(String.format("Places disponibles : %d", trajet.getAvailableSeats()));
                            placesLabel.setStyle("-fx-text-fill: #28A745; -fx-font-weight: bold;");
                            container.getChildren().add(placesLabel);
                        }
                    } catch (SQLException e) {
